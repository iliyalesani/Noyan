...
?>
 <?php
		$dbResult = mysql_query("SELECT * FROM mtblmovie WHERE  ID = ".$product_id,$dlink);
	...
	if(document.getElementById('movie').value.length>0)
        {
		  if (!document.getElementById('movie').value.match(/\.(avi|mp4|wmv|flv)$/))
		   {
			   result+='پسوند ویدیوی انتخاب شده نا صحیح است \n';
           }
     	}
...
<div id="main_column" >
<div class="mgbox">ویرایش ویدیو</div>
 <!--**********************************************main***************************************-->      
 



<form action="update_movie.php" method="post" style="font-size:16px; line-height:22px;"  enctype="multipart/form-data">
...
<input name="fl" type="file"  id="fl"/></td>
 ...
 <tr>
    <td>ویدیو:</td>
    <td><input type="hidden" name="MAX_FILE_SIZE" value="9900000"  />
<input name="movie" type="file"  id="movie"/></td>
  </tr>
    <tr>
    <td>متن اصلی:</td>
...
<?php
			   ...
			   $dbresult = mysql_query("select * from mtblsubmenu  where parentid=4",$dlink);
