...
?>
 <?php
		$dbResult = mysql_query("SELECT * FROM mtblpost WHERE  ID = ".$product_id,$dlink);
...
<div id="main_column" >
<div class="mgbox">ویرایش مقاله</div>
 <!--**********************************************main***************************************-->      
 



<form action="updatepost.php" method="post" style="font-size:16px; line-height:22px;"  enctype="multipart/form-data">
...
<table width="100%" bordercolor="#cccccc"  border="1" rules="rows" cellpadding="5" >
  <tr>
   ...
   <tr>
    <td>متن اصلی:</td>
    <td><textarea name="area2" class="ckeditor"    style="width:80%; height:500px; background-color:#FFF; background-image:none" > <?php echo $sub['longtext'] ?></textarea></td>
  </tr>
      <tr>
    <td>عکس قبلی:</td>
...
<option <?php if ($sub['published']==0)echo 'selected="selected"';?> value="0">عدم انتشار</option>
    ...
    <td></td>
    <td>
      <input type="submit" value="ارسال" id="submit" name="submit" onclick="return confirm_send();"   class="formstyleb1" />
