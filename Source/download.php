...
<?php
		...
		{
			mysql_query("update mtbldownload set view=view+1 where id=".$_GET['id'],$dlink);
              $dbresultpost = mysql_query("select * from mtbldownload where  published = 1 and id=".$_GET['id']  ,$dlink);
			  $post=mysql_fetch_assoc($dbresultpost);
  ?>              
                
				<!-- ENDS title -->
				<h1 class="contenth1 sourcecodeli" style="background-color:#333"><?php echo $post['titr'];?></h1>
				<!-- Portfolio -->
 ...
 <?php                    
      if ($post['link1'] <> "" || $post['link2'] <> "" || $post['link3'] <> "")
	  { 
		 echo('<div class="downloadpanel ">
		   <h3>  لینک های دانلود </h3>');
		   
		   if ($post['link1'] <> "")
		   echo ('
		   <a style="color:#fff;" href="'.$post['link1'].'">'.$post['linktitle1'].'</a>');
		   if ($post['linkpass1'] <> "")
		   ...
		   echo '<br/><br/>';
		    ...
		    if ($post['link2'] <> "")
		   echo ('
		   <a style="color:#fff;" href="'.$post['link2'].'"> '.$post['linktitle2'].'</a>');
		   if ($post['linkpass2'] <> "")
		  ...
		  if ($post['link3'] <> "")
		   echo ('
		   <a  style="color:#fff;" href="'.$post['link3'].'">  '.$post['linktitle3'].'</a>');
		   if ($post['linkpass3'] <> "")
...
<?php   
		if (isset($_GET['id']) && is_numeric($_GET['id']) )
		{
              $dbresultcomment = mysql_query("select * from mtblcomment where  readed = 1 and pagename='download.php' and postid=".$_GET['id']  ,$dlink);
