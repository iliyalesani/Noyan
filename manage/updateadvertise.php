...
$setfile=' ';
  if($_FILES['fl']['size']>10)
  {
		...
		$setfile="`image`= 'images/advertise/".$filename."' ,";
		
		
		//delete old logo from directory
			$dbresult= mysql_query("select `image` from `mtbladvertise` where id=".$product_id,$dlink);
...
`link` = '".$_POST['link']."'
  ...
  window.location='manage.php?type=<?php echo $type_back ?>&msg=تبلیغ مورد نظر با موفقیت ویرایش شد';
  </script>		 
  <?php
}
else
