...
$setfile2=' ';
  if($_FILES['fl']['size']>10)
  {
		...
		//delete old logo from directory
			$dbresult= mysql_query("select `image` from `mtblmovie` where id=".$product_id,$dlink);
			$filename_delete=mysql_fetch_assoc($dbresult);
			unlink('../'.$filename_delete['image']);
 }
   if($_FILES['movie']['size']>10)
  {
			  $uploadedfile=$_FILES['movie']['name'];
		$filepath='../movies/';
		$filename=$_FILES['movie']['name'];
			...
			$filename=$i.$filename;
		move_uploaded_file($_FILES['movie']['tmp_name'], $filepath.$filename);
		$setfile2="`movie`= 'movies/".$filename."' ,";
		
		
		//delete old logo from directory
			$dbresult= mysql_query("select `movie` from `mtblmovie` where id=".$product_id,$dlink);
			$filename_delete=mysql_fetch_assoc($dbresult);
			unlink('../'.$filename_delete['movie']);
 }

	                
						$sql="update  `mtblmovie` set 
`titr` ='".$_POST['titr']."',
